<template>
  <div class="dashboard-container container-fluid">
    <h2 class="mb-4">{{ t('managerDashboard.title') }} | {{ authStore.userRole }}</h2>
        ...
        <div v-if="profileId">
          <div class="row gy-4">
            ...
            <div class="col-md-6 col-lg-4">
              <div class="card h-100 shadow-sm">
                <div class="card-header bg-fadaa-yellow">
                  <h5 class="mb-0"><i class="bi bi-file-earmark-text-fill me-2"></i>{{ t('managerDashboard.renewals.title') }}</h5>
                </div>
                <div class="card-body">
                  ...
                  <div v-if="renewals.length === 0" class="text-center text-muted mt-2">{{ t('managerDashboard.renewals.empty') }}</div>
                </div>
              </div>
            </div>
            ...
            <div class="col-md-6 col-lg-4">
              <div class="card h-100 shadow-sm">
                <div class="card-header bg-fadaa-yellow">
                  <h5 class="mb-0"><i class="bi bi-hourglass-split me-2"></i>{{ t('managerDashboard.expiringContracts.title') }}</h5>
                </div>
                <div class="card-body">
                  <ul class="list-group list-group-flush">
                  ...
                  <div v-if="expiringContracts.length === 0" class="text-center text-muted mt-2">{{ t('managerDashboard.expiringContracts.empty') }}</div>
                </div>
              </div>
            </div>
            ...
            <div class="col-md-6 col-lg-4">
              <div class="card h-100 shadow-sm">
                <div class="card-header bg-fadaa-yellow">
                  <h5 class="mb-0"><i class="bi bi-person-lines-fill me-2"></i>{{ t('managerDashboard.prospects.title') }}</h5>
                </div>
                <div class="card-body">
                  <ul class="list-group list-group-flush">
                    <li v-for="prospect in prospects" :key="prospect.id" class="list-group-item d-flex justify-content-between align-items-center">
                      ...
                      <button class="btn btn-sm btn-outline-fadaa-orange"><i class="bi bi-telephone-fill me-1"></i>{{ t('managerDashboard.prospects.contact') }}</button>
                    </li>
                  </ul>
                  <div v-if="prospects.length === 0" class="text-center text-muted mt-2">{{ t('managerDashboard.prospects.empty') }}</div>
                </div>
              </div>
            </div>
            ...
            <div class="col-md-6 col-lg-8">
              <div class="card h-100 shadow-sm">
                <div class="card-header bg-fadaa-yellow">
                  <h5 class="mb-0"><i class="bi bi-building-check me-2"></i>{{ t('managerDashboard.offices.title') }}</h5>
                </div>
                <div class="card-body">
                  <div class="table-responsive">
                    <table class="table table-sm table-hover">
                      <thead class="table-light">
                        <tr>
                          <th>{{ t('managerDashboard.offices.table.name') }}</th>
                          <th>{{ t('managerDashboard.offices.table.status') }}</th>
                          <th>{{ t('managerDashboard.offices.table.capacity') }}</th>
                          <th>{{ t('managerDashboard.offices.table.branch') }}</th>
                          <th>{{ t('managerDashboard.offices.table.actions') }}</th>
                        </tr>
                      </thead>
                      <tbody>
                  ...
                  <div v-if="offices.length === 0" class="text-center text-muted mt-2">{{ t('managerDashboard.offices.empty') }}</div>
                </div>
              </div>
            </div>
            ...
            <div class="col-md-6 col-lg-4">
              <div class="card h-100 shadow-sm">
                <div class="card-header bg-fadaa-yellow">
                  <h5 class="mb-0"><i class="bi bi-check2-square me-2"></i>{{ t('managerDashboard.tasksApprovals.title') }}</h5>
                </div>
                <div class="card-body">
                  <div v-if="tasksLoading" class="text-muted small mb-2">{{ t('managerDashboard.tasksApprovals.loading') }}</div>
                  <ul class="list-group list-group-flush">
                    <li v-for="task in pendingTasks" :key="task.id" class="list-group-item d-flex justify-content-between align-items-center">
                      <div class="me-2">
                        ...
                        <div class="text-muted small">
                          {{ t('managerDashboard.tasksApprovals.due') }}: {{ formatDate(task.due_date) }} ·
                          {{ t('managerDashboard.tasksApprovals.priority') }}: {{ task.priority }} ·
                          {{ t('managerDashboard.tasksApprovals.status') }}: {{ task.status }}
                        </div>
                      </div>
                      <div class="ms-2">
                        <button class="btn btn-sm btn-success me-1" @click="markTaskCompleted(task)"><i class="bi bi-check-lg me-1"></i>{{ t('managerDashboard.tasksApprovals.complete') }}</button>
                        <button class="btn btn-sm btn-outline-secondary" @click="goToTasks"><i class="bi bi-box-arrow-up-right me-1"></i>{{ t('managerDashboard.tasksApprovals.viewAll') }}</button>
                      </div>
                    </li>
                  </ul>
                  <div v-if="!tasksLoading && pendingTasks.length === 0" class="text-center text-muted mt-2">{{ t('managerDashboard.tasksApprovals.noPending') }}</div>
                </div>
              </div>
            </div>
...
const markTaskCompleted = async (task) => {
  try {
    ...
    showSuccessToast(t('managerDashboard.tasksApprovals.completedMsg'));
    await fetchTasks();
  } catch (e) {
    // Error toast is handled globally by ApiClient interceptor
  }
...
const loadManagerDashboard = async (profileId = null) => {
  if (!profileId) return;
...
const handleProfileUpdate = (profileId) => {
  ...
  loadManagerDashboard(profileId);
...
const exportData = async (format) => {
  ...
  try {
    const exportConfig = {
      ...
      type: 'manager', // Differentiates from admin/financial reports
      data: {
